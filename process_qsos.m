...
load(sprintf('%s/learned_zqso_only_model_outdata_full_%s_norm_%d-%d',             ...
...
for quasar_ind = q_ind_start:num_quasars %quasar list
    ...
    parfor i = 1:num_zqso_samples       %variant redshift in quasars
        ...
        this_normalized_noise_variance_bw = 0; %this_normalized_noise_variance(ind_bw);
        ...
        this_normalized_noise_variance_rw = 0; %this_normalized_noise_variance(ind_rw);
...
filename = sprintf('%s/processed_zqso_only_qsos_%s-%s_%d-%d_%d-%d_outdata_full_nonoise_oc%d', ...
