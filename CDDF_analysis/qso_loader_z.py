...
from .qso_loader import QSOLoader, make_fig, GPLoader
...
class QSOLoaderZ(QSOLoader):
    ...
    def plot_this_mu(self, nspec, suppressed=True, num_voigt_lines=3, num_forest_lines=6, 
            ...
            normalization_mins : List = [1156, 1509],
            normalization_maxs : List = [1276, 1589]):
        '''
        ...
        this_flux, this_noise_variance = self.normalisation(
            ...
            normalization_mins, normalization_maxs)
    ...
    def normalisation(rest_wavelengths, flux, noise_variance, 
        ...
        '''
        # init an false array
        ind = np.zeros(rest_wavelengths.shape, dtype=np.bool)

        # multiple normalizations
        for normalization_min_lambda, normalization_max_lambda in zip(
                normalization_mins, normalization_maxs):

            #normalizing here
            ind = ind | ( (rest_wavelengths >= normalization_min_lambda) & 
                (rest_wavelengths <= normalization_max_lambda))

        assert ind.sum() < len(rest_wavelengths)
