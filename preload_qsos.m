...
load(sprintf('%s/zqso_smallz_catalog', processed_directory(release)), ...
    variables_to_load{:});
...
variables_to_save = {'loading_min_lambda', 'loading_max_lambda', ...
                     ...
                     'all_normalizers'};
save(sprintf('%s/preloaded_zqso_smallz_qsos', processed_directory(release)), ...
     variables_to_save{:}, '-v7.3');

% write new filter flags to catalog
save(sprintf('%s/zqso_smallz_catalog', processed_directory(release)), ...
     'filter_flags', '-append');
